...
import random
from search_spaces.sample import random_sample
...
class EAOptimizer:
    ...
    def mutate_width(self, cs, architecture):
        if random.random() < self.mutation_prob_width:
            architecture = cs.sample_arch_latin_hypercube(20)
        return architecture
    ...
    def generate_initial_population(self, cs):
        P = [cs.sample_arch_latin_hypercube(20)] * self.population_size
        print(len(P))
        ...
        while (not self.satisfied_constrained(P)):
            for i, s in enumerate(slope):
                if s > self.T_AVM:
                    P[i] = cs.sample_arch_latin_hypercube(20)
        return P
